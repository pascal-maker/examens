...
def make_env(seed: int = 0):
    ...
    Ensures reproducibility by seeding env, action space, numpy, and random.
    """
...
def run_episode(env: gym.Env, policy_fn: Callable[[np.ndarray], int], max_steps: int = 1000) -> int:
    ...
    Returns the total cumulative reward (equal to steps survived in CartPole).
    """
    obs, _ = env.reset()
    total = 0
...
def eval_policy(env_maker: Callable[[], gym.Env],
    ...
    Creates a fresh environment for the batch (though seed=0 inside run loops might need care if parallelized).
    """
    env = env_maker()
...
def plot_hist(scores: List[float], title: str, out: str = None):
    """
    Generate and save (or show) a histogram of episode scores.
...
def random_policy(_: np.ndarray) -> int:
    """Always returns a random valid action (0 or 1)."""
    return np.random.randint(0, 2)

def angle_policy(obs: np.ndarray) -> int:
    """
    Heuristic policy based on pole angle.
    obs: [x, x_dot, theta, theta_dot]
    If pushing right (1) counteracts a right tilt (theta > 0), this logic actually
    pushes *towards* the fall (unstable). Correct logic usually pushes *into* the fall to correct it.
    Note: CartPole definitions: theta>0 means pole is leaning Right. Action 1 pushes Cart Right.
    So if leaning right, pushing right moves cart under the pole? No, actually pushing right accelerates base right,
    causing pole to tip LEFT relative to base (inertial frame). So action 1 corrects positive theta.
    """
    theta = obs[2]
...
def linear_policy(weights: np.ndarray) -> Callable[[np.ndarray], int]:
    """
    Returns a policy function that computes dot product of weights and observation.
    Action 1 if w*x >= 0, else 0.
    """
...
def avg_return_for_weights(w: np.ndarray,
    ...
    """
    Helper to calculate average reward for a specific weight vector.
...
def run_random(episodes: int, plot: str = None):
    """Execution for Part 1: Random Baseline."""
...
def run_angle(episodes: int, plot: str = None):
    """Execution for Part 2: Angle-based Heuristic."""
...
def random_search(iters: int, per_iter: int, w_scale: float = 1.0,
    ...
    """
    Part 3: Random Search.
    Simply samples weights from a normal distribution repeatedly and keeps the best.
    Returns (best_weights, history).
    """
    ...
    for t in range(1, iters+1):
        # Sample completely new random weights
...
def scatter_3d(history: List[SearchRecord], out: str = None):
    """
    Visualizes the search space of Random Search.
    Selects top-3 dimensions by weight magnitude and plots performance (red > 100).
    """
    # Determine importance by absolute weight magnitude
    W = np.stack([rec.w for rec in history], axis=0)
    A = np.abs(W).mean(0)
    idx = np.argsort(-A)[:3]  # top-3 indices
    w123 = W[:, idx]
...
def hill_climb(iters: int, per_iter: int, sigma: float = 0.1,
    ...
    """
    Part 4: Hill Climbing.
    Starts with random weights. In each step, adds noise to current weights.
    If the new candidate is better, move to it. Otherwise stay.
    Local optimization method, can get stuck in local optima.
    """
    rng = np.random.default_rng(seed)
    env_maker = lambda: make_env(seed)
...
def accept_prob(delta: float, T: float) -> float:
    # Accept downhill moves with probability e^(delta/T)
    # Note: delta here is (new - old). If new < old, delta is negative.
...
def simulated_annealing(iters: int, per_iter: int,
    ...
    """
    Part 5: Simulated Annealing.
    Like Hill Climbing, but can accept worse solutions to escape local optima.
    Probability of accepting worse solution decreases as Temperature (T) drops.
    Includes 'adaptive' option to scale noise (sigma) based on recent success/failure.
    """
    rng = np.random.default_rng(seed)
    env_maker = lambda: make_env(seed)
    ...
    for t in range(1, iters+1):
        ...
        if val >= cur:
            ...
            take = True
            if adaptive:
                sigma = max(min_sigma, sigma * 0.5)  # Reward: fine-tune (shrink noise)
        else:
            # Maybe accept degradation
            if rng.random() < accept_prob(delta, T):
                take = True
                if adaptive:
                    sigma = min(max_sigma, sigma * 2.0)  # Failure: explore broader (grow noise)
...
def test_weights(w: np.ndarray, episodes: int, seed: int = 101) -> Tuple[float, List[float]]:
    """Final verification of finding weights on a fresh seed."""
    env_maker = lambda: make_env(seed)
...
def main():
    ...
    p.add_argument("--episodes", type=int, default=1000, help="Episodes for final evaluation")
    p.add_argument("--plot", type=str, default=None, help="Output filename for plots (png)")
    p.add_argument("--iters", type=int, default=1000, help="Iterations for search algorithms")
    p.add_argument("--per_iter", type=int, default=20, help="Episodes to average per iteration estimate")
    p.add_argument("--sigma", type=float, default=0.1, help="Noise std dev for mutations")
    p.add_argument("--t0", type=float, default=1.0, help="Initial temperature for SA")
    p.add_argument("--cool", type=float, default=0.995, help="Cooling rate for SA")
    p.add_argument("--adaptive", action="store_true", help="Enable adaptive noise scaling for SA")
    p.add_argument("--scatter", type=str, default=None, help="Output filename for 3D scatter plot")
    args = p.parse_args()
