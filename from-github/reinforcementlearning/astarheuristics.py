...
W, H = 900, 700
COLS, ROWS = 45, 35
CELL = min((W-40)//COLS, (H-40)//ROWS) # Calculate square cell size
OX, OY = 20, 20 # Offsets for margins
...
TEXT = (225, 228, 235)
MUTED = (165, 170, 180)
ROUND = 6 # Radius for rounded rectangles
...
class Node:
    """Represents a coordinate on the grid. Immutable and hashable."""
    x: int
    y: int
    def __iter__(self): # Allows for tuple unpacking: x, y = node
        yield self.x; yield self.y

def in_bounds(x:int,y:int)->bool:
    """Return True when the grid index is within valid dimensions."""
    return 0<=x<COLS and 0<=y<ROWS

def to_cell(mx:int,my:int)->Optional[Node]:
    """Translate mouse pixel coordinates into a grid node or None if outside."""
    if not (OX <= mx < OX+GRID_W and OY <= my < OY+GRID_H):
        return None
    return Node((mx-OX)//CELL, (my-OY)//CELL)

def rect_of(n:Node)->pygame.Rect:
    """Compute the pygame.Rect used to draw a cell for node n."""
    return pygame.Rect(OX + n.x*CELL+1, OY + n.y*CELL+1, CELL-2, CELL-2)

# --------------------------------------------------------------------------- #
# HEURISTICS (Task 1)
# --------------------------------------------------------------------------- #
# Heuristics estimate the cost from point p1 to p2.

def euclidean_heuristic(p1,p2)->float:
    """Straight-line distance. Good for any-angle movement."""
    x1,y1=(p1.x,p1.y) if hasattr(p1,"x") else p1
    x2,y2=(p2.x,p2.y) if hasattr(p2,"x") else p2
    return math.hypot(x1-x2,y1-y2)

def manhattan_heuristic(p1,p2)->float:
    """Sum of x and y differences. Standard for 4-way grid movement."""
    x1,y1=(p1.x,p1.y) if hasattr(p1,"x") else p1
    x2,y2=(p2.x,p2.y) if hasattr(p2,"x") else p2
    return abs(x1-x2)+abs(y1-y2)

def chebyshev_heuristic(p1,p2)->float:
    """Max of x or y difference. Standard for 8-way grid movement."""
    x1,y1=(p1.x,p1.y) if hasattr(p1,"x") else p1
    x2,y2=(p2.x,p2.y) if hasattr(p2,"x") else p2
...
def weighted_euclidean_heuristic(p1,p2,weight:float=1.0)->float:
    """
    Multiplies Euclidean distance by a weight.
    w > 1: Greedier, faster, but may not find optimal path (suboptimal).
...
def step_cost(a:Node,b:Node)->float:
    """Cost to move between neighbors. Orthogonal=1, Diagonal=sqrt(2)."""
    return math.sqrt(2) if (a.x!=b.x and a.y!=b.y) else 1.0

def corner_cut(grid, a:Node, b:Node)->bool:
    """Block diagonal moves if they would 'clip' through a wall corner."""
...
def astar(grid,start:Node,goal:Node,diag:bool,heuristic_fn:Callable[[Node,Node],float]):
    """
    A* Generator. Yields algorithm state at each step for visualization.
    state: (current_node, seen_set, closed_set, came_from_dict)
    """
    openh:List[Tuple[float,int,Node]]=[] # Min-heap for nodes to explore
    g:Dict[Node,float]={start:0.0}       # Cost from start to node
    came:Dict[Node,Optional[Node]]={start:None} # Parent pointers for path reconstruction
    seen:Set[Node]={start}               # Nodes that have been added to openh
    closed:Set[Node]=set()               # Nodes that have been expanded
    t=0 # Tie-breaker for heap stability
    ...
    while openh:
        ...
        # Yield state for resizing/drawing
        ...
        for nb in neighbors(cur,diag):
            if grid[nb.x][nb.y]: continue # Wall
            if diag and corner_cut(grid,cur,nb): continue # Invalid diagonal
...
def reconstruct(came:Dict[Node,Optional[Node]], end:Node)->List[Node]:
    """Walk backwards from the goal to start to produce the path list."""
...
# --------------------------------------------------------------------------- #
def empty_grid()->list:
    """Returns a grid filled with 0s (empty space)."""
...
def scattered_obstacles(density=0.20, start:Node=None, goal:Node=None)->list:
    """Random wall placement. Retries generation until a valid path exists."""
    rnd=random.Random(42) # Deterministic seed for consistent testing
    while True:
        grid=empty_grid()
        for x in range(COLS):
            for y in range(ROWS):
        ...
        # Verify connectivity using Dijkstra
        ok,_=dijkstra_length(grid,start,goal,diag=True)
        if ok: return grid

def maze_dfs(start:Node, goal:Node)->list:
    """
    Generates a maze using Randomized Depth-First Search (Iterative).
    ...
    while stack:
        ...
        if nbrs:
            ...
            # Remove wall between current cell (cx, cy) and neighbor (nx, ny)
    ...
    # Fallback to clear path if maze generation accidentally ignored goal area
    ok,_=dijkstra_length(grid,start,goal,diag=True)
    if not ok:
        x,y=start.x,start.y
...
def linear_barriers(kind:str)->list:
    """Deterministic line-based obstacles to illustrate detours (e.g. L-shape)."""
    g=empty_grid()
    if kind=="horizontal":
        y=ROWS//2
        for x in range(COLS):
...
def geometric_shape(kind:str)->list:
    """Render geometric blocker patterns (Squares, Circles, Spirals) for stress testing."""
...
# Pattern Registry: Maps family name to list of variant names
PATTERNS = {
    "off": [None],
...
def main():
    ...
    def apply_pattern(family:str, variant:Optional[str])->list:
        """Dispatcher to generate selected grid pattern."""
    ...
    while True:
        for e in pygame.event.get():
            ...
            if e.type==pygame.KEYDOWN:
                ...
                if e.key==pygame.K_SPACE: running=not running # Pause/Resume
                if e.key==pygame.K_r: reset() # Reset Algorithm
                if e.key==pygame.K_c:
                    grid = empty_grid(); reset() # Clear Grid
                if e.key==pygame.K_d:
                    diag = not diag; reset() # Toggle Diagonal

                # Heuristic hotkeys (1-4)
                ...
                # Benchmarks (Output to terminal)
                if e.key==pygame.K_b:
                    run_benchmarks()

                # Pattern cycling (Task 3)
                if e.key==pygame.K_p: # Cycle family
                    ...
                    grid=apply_pattern(family,variant)
                    reset()
                if e.key==pygame.K_COMMA:   # Prev variant within family
                    v_idx=(v_idx-1)%len(PATTERNS[family])
                    variant=PATTERNS[family][v_idx]
                    grid=apply_pattern(family,variant)
                    reset()
                if e.key==pygame.K_PERIOD:  # Next variant within family
                    v_idx=(v_idx+1)%len(PATTERNS[family])
                    variant=PATTERNS[family][v_idx]
                    grid=apply_pattern(family,variant)
                    reset()
            ...
            if e.type in (pygame.MOUSEBUTTONDOWN, pygame.MOUSEMOTION):
                buttons = pygame.mouse.get_pressed(3) # [Left, Mid, Right]
                pos = pygame.mouse.get_pos()
                cell = to_cell(*pos)
                keys = pygame.key.get_pressed()
                if cell:
                    # 'S'+Click for Start, 'G'+Click for Goal
        ...
        if running and gen:
            try:
                ...
                if tag=="done":
                    ...
                    # Live Preview: Show path to "best looking" node in open set
                    heur = make_heur()
                    if open_set:
...
def dijkstra_length(grid, start:Node, goal:Node, diag:bool)->Tuple[bool,float]:
    """
    Run Dijkstra (Uniform Cost Search) to find the absolute true shortest path cost.
    Used as the 'Optimal' baseline for benchmarks.
    """
...
def astar_run(grid,start,goal,diag,heur_fn):
    """
    Execute A* logic without yielding/visualization (Headless Mode).
    Returns metrics: path length, time elapsed, nodes explored.
    """
...
def run_benchmarks():
    """
    Compare heuristics across multiple scenarios and print a table.
    Scenarios include basic grids and complex obstacle patterns.
    """
    random.seed(42)
    start = Node(5,5)
    ...
    WEIGHTS=[0.5,1.0,1.5,2.0]
    # List of heuristics to test against each scenario
    heuristics = [
...
if __name__=="__main__":
    _maybe = _maybe_run_bench_from_argv()
    if not _maybe:
        main()
